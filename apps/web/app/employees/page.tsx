    ...
    <AppShell title="Colaboradores" subtitle="Cadastre e mantenha atualizada a equipe operacional.">
      ...
      <section className="card" style={{ marginBottom: 16 }}>
        ...
        <form className="employee-form" onSubmit={submit}>
          ...
          <div className="field"><label>Filial</label><select required value={unitId} onChange={(e) => setUnitId(e.target.value)}><option value="">Selecione a filial</option>{units.map((unit) => <option key={unit.id} value={unit.id}>{unit.name} · {unit.code}</option>)}</select></div>
