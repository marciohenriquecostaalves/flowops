...
export class JobTitlesService {
  ...
  async create(tenantId: string, actorUserId: string, dto: CreateJobTitleDto) {
    try { const jobTitle = await this.prisma.jobTitle.create({ data: { tenantId, name: dto.name } }); await this.audit(tenantId, actorUserId, 'JOB_TITLE_CREATED', jobTitle.id, jobTitle.name); return jobTitle; }
  ...
  async update(tenantId: string, actorUserId: string, id: string, dto: UpdateJobTitleDto) {
    ...
    try { const updated = await this.prisma.jobTitle.update({ where: { id }, data: dto }); await this.audit(tenantId, actorUserId, 'JOB_TITLE_UPDATED', id, updated.name); return updated; }
    catch { throw new ConflictException('Não foi possível atualizar o cargo'); }
  }
  private audit(tenantId: string, userId: string, action: string, entityId: string, name: string) { return this.prisma.auditLog.create({ data: { tenantId, userId, action, entity: 'JOB_TITLE', entityId, metadata: { name } } }); }
}
